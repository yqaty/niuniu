...
- 📸 **截图识别**：自动定位棋盘区域、检测网格线、聚类颜色
- 🧠 **约束求解**：CP-SAT 秒级求解
- 🖼️ **答案标注**：在原图上直接标记牛的位置
- 📱 **手机友好**：响应式网页，支持拍照/选图上传
- 🔄 **最新结果展示**：网页实时显示最近一次求解结果（5 秒轮询），方便多人共用同一服务
...
|------|------|------|
| `/` | GET | 手机友好的上传页面 |
| `/solve` | POST | 上传截图求解，返回标注图片 |
| `/latest` | GET | 获取最新求解结果（JSON + base64 图片） |
| `/health` | GET | 健康检查 |
...
浏览器打开 `http://<服务器IP>:8080`，上传截图，点击求解。页面底部会展示最新一次的求解结果。
